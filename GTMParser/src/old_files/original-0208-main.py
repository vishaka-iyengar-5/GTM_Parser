#!/usr/bin/env python3
"""
GTM Detector - Complete Implementation
...
from playwright.async_api import Page, Response, async_playwright #Controlling a real browser to interact with websites, capture network traffic, and simulate user behavior
...
class GTMDetector:
    ...
    async def _detect_javascript_gtm(self, page: Page):
        ...
        try:
            ...
            # ENHANCED gtag calls detection - Shows consent implementation and event tracking - Searches through all script tags to find ALL gtag calls
            ...
            () => {
                ...
                scripts.forEach(script => {
                    ...
                    // Extract all gtag calls by type
...
async def test_gtm_detector():
    ...
    test_urls = [
        ...
        "https://github.com",            # Likely has GTM
        "https://stackoverflow.com",     # Popular site, likely has GTM
    ]
...
async def main():
    ...
    if len(sys.argv) > 1:
        url = sys.argv[1]
        await analyze_single_url(url)
    else:
        # Run test suite
        success = await test_gtm_detector()
        if success:
            print("\n🎉 GTM Detection test completed successfully!")
        else:
            print("\n❌ GTM Detection test failed!")
            sys.exit(1)
